...
const getSourceMap = async (url, content) => {
	...
	for (let i = lines.length - 1; i >= 0 && !sourceMapUrl; i--) {
		...
		if (result) sourceMapUrl = result[1] || result[2];
	}
...
const parseStackFrame = async (frame, maxDepth) => {
	...
	const results = await Promise.all(pendingSources.map(async ({ col: columnNumber = 0, file: fileName, line: lineNumber }) => {
		...
		try {
			...
			if (response.ok) {
				...
				if (sourcemap) {
					...
					const originalSource = (result && typeof result.source === "string" ? result.source : void 0) || void 0;
					return {
...
const describeFiber = (fiber, childFiber) => {
	...
	switch (tag) {
		...
		case HostSingletonTag: return describeBuiltInComponentFrame(fiber.type);
