...
const require_extract_element_css = require('./extract-element-css-DMtrzGGR.cjs');
...
const getReactProps = (element) => {
	const fiber = require_freeze_updates.Z(element);
	if (!fiber) return [];
	let compositeFiber = fiber.return;
	while (compositeFiber) {
		if (require_freeze_updates.ve(compositeFiber)) break;
	...
	return Object.entries(fiberProps).filter(([propKey]) => !HIDDEN_PROP_KEYS.has(propKey) && !propKey.startsWith("__")).slice(0, 6).map(([propKey, propValue]) => {
		...
		if (formatted.length > 40) formatted = `${formatted.slice(0, 40 - 1)}…`;
		return {
			label: propKey,
			value: formatted
		};
...
const STATEFUL_HOOK_TYPES = new Set(["useState", "useReducer"]);
...
const initRenderTimeline = () => {
	...
	require_freeze_updates.Me({ onCommitFiberRoot(_rendererID, root) {
		...
		if (!rootFiber) return;
		const { totalTime } = require_freeze_updates.Se(rootFiber);
		const componentTypes = /* @__PURE__ */ new Set();
		require_freeze_updates.Oe(root, (fiber) => {
			if (require_freeze_updates.ve(fiber) && require_freeze_updates.we(fiber.type)) componentTypes.add(fiber.type);
		});
		if (componentTypes.size === 0) return;
		commitBuffer.push({
...
const getComponentType = (element) => {
	...
	let compositeFiber = fiber.return;
	while (compositeFiber) {
		if (require_freeze_updates.ve(compositeFiber)) return compositeFiber.type;
		compositeFiber = compositeFiber.return;
	}
	return null;
};
const getTimelineForElement = (element) => {
	...
	const commits = commitBuffer.filter((entry) => entry.componentTypes.has(componentType)).map((entry) => ({
	...
	return {
		commits,
		totalRenderCount: commits.length
	};
};
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		const handlePointerMove = (clientX, clientY) => {
			if (!isEnabled() || isPromptMode() || isInspectMode() || isFrozenPhase() || store.contextMenuPosition !== null) return;
			actions.setPointer({
				x: clientX,
				y: clientY
			});
		...
		const inspectPropertiesState = (0, solid_js.createMemo)(() => {
			...
			const filePath = store.selectionFilePath;
			const lineNumber = store.selectionLineNumber;
			const source = filePath ? `${filePath.split("/").pop()}${lineNumber ? `:${lineNumber}` : ""}` : "";
			return {
				width: rect.width,
				height: rect.height,
				className: element.className || "",
				reactProps: getReactProps(element),
				hooks: getHooksState(element),
				renderedBy: getRenderedBy(element),
				source,
				timeline: getTimelineForElement(element),
				isVisible: true
			};
		});
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-bagN85My.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
