...
const require_extract_element_css = require('./extract-element-css-BE7h4Ch-.cjs');
...
//#endregion
//#region src/utils/get-rendered-by.ts
const getRenderedBy = (element) => {
	const fiber = require_freeze_updates.Z(element);
	if (!fiber) return "";
	let ownerName = "";
	let compositeFiber = fiber.return;
	while (compositeFiber) {
		if (require_freeze_updates.ve(compositeFiber)) {
			const name = require_freeze_updates.we(compositeFiber.type);
			if (name) {
				ownerName = name;
				break;
			}
	...
	const renderer = [...require_freeze_updates.d][0];
	const rendererLabel = renderer ? `${renderer.rendererPackageName}@${renderer.version}` : "";
	if (ownerName && rendererLabel) return `${ownerName} · ${rendererLabel}`;
	return ownerName || rendererLabel;
};
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CeCBr7my.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
