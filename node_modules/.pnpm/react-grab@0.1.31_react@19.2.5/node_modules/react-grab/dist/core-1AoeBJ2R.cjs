...
const require_extract_element_css = require('./extract-element-css-CchtITXx.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Cp8W_Vpj.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
