...
const require_extract_element_css = require('./extract-element-css-CF6T5fBp.cjs');
...
const getHooksState = (element) => {
	...
	return require_freeze_updates.kt(compositeFiber).filter((hookNode) => hookNode.isStateEditable).slice(0, 8).map((hookNode) => ({
		label: hookNode.name,
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Ba0AWwqv.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
