...
const require_extract_element_css = require('./extract-element-css-CQx4Biqa.cjs');
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Ds21virZ.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
