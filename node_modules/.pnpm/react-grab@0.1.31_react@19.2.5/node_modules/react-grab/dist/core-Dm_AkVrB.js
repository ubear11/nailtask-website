 ...
 */
import { A as Oe, B as hideFromThirdParties, D as kt, E as Mt, F as we, G as DEFAULT_ACTION_ID, H as getTagName, I as z, L as nativeCancelAnimationFrame, N as Z, O as zt, P as ve, R as nativeRequestAnimationFrame, S as getNearestComponentName, T as resolveSource, U as ARROW_KEYS, V as detectCspNonce, X as FEEDBACK_DURATION_MS, _ as isRootElement, a as freezeGlobalAnimations, at as OFFSCREEN_POSITION, b as getElementContext, bt as ZOOM_DETECTION_THRESHOLD, c as clearElementPositionCache, d as freezePseudoStates, f as resumePointerEventsFreeze, g as isValidGrabbableElement, gt as TOOLBAR_DEFAULT_POSITION_RATIO, h as clearVisibilityCache, i as freezeAnimations, it as NEXTJS_REVALIDATION_DELAY_MS, j as Se, k as Me, l as getElementAtPosition, m as unfreezePseudoStates, n as logRecoverableError, nt as MODIFIER_KEYS, o as unfreezeGlobalAnimations, p as suspendPointerEventsFreeze, q as DRAG_SELECTION_COVERAGE_THRESHOLD, r as freezeAllAnimations, rt as MOUNT_ROOT_RECHECK_DELAY_MS, s as openFile, t as freezeUpdates, tt as MAX_SESSION_STORAGE_SIZE_BYTES, u as getElementsAtPoint, v as checkIsNextProject, w as getStackContext, xt as Z_INDEX_OVERLAY, y as getComponentDisplayName, yt as VERSION, z as waitUntilNextFrame } from "./freeze-updates-BFaxCqlc.js";
import { a as isMac, g as isElementConnected, h as isKeyboardEventTriggeredByInput, i as isEventFromOverlay, m as hasTextSelectionOnPage, n as saveToolbarState, p as hasTextSelectionInInput, r as resolveActionEnabled, s as clampToRange, t as loadToolbarState } from "./state-BdkAwq7U.js";
import { n as extractElementCss, r as createElementSelector, t as disposeBaselineStyles } from "./extract-element-css-7ePNn7Qb.js";
...
const findNearestCompositeFiber = (element) => {
	const hostFiber = Z(element);
	if (!hostFiber) return null;
	let currentFiber = hostFiber.return;
	while (currentFiber) {
		if (ve(currentFiber)) return currentFiber;
...
const buildHookRows = (hooksTree, hookNames) => collectEditableHooks(hooksTree).slice(0, 8).map((hookNode) => {
	return {
		label: (hookNode.hookSource && hookNames ? hookNames.get(Mt(hookNode.hookSource)) : void 0) ?? hookNode.name,
...
const resolveHookNames = async (element) => {
	...
	const hookNames = await zt(hooksTree);
...
const initRenderTimeline = () => {
	...
	Me({ onCommitFiberRoot: (_rendererID, root) => {
		...
		Oe(root, (fiber) => {
			if (ve(fiber) && we(fiber.type)) {
				const fiberId = z(fiber);
				const { selfTime } = Se(fiber);
...
const getTimelineForElement = (element) => {
	...
	const inspectedFiberId = z(compositeFiber);
...
const init = (rawOptions) => {
	...
	return createRoot((dispose) => {
		...
		const [resolvedHookState, setResolvedHookState] = createSignal(void 0);
		createEffect(on(() => isInspectMode() ? effectiveElement() : null, (element) => {
		...
		if (pluginRegistry.store.theme.enabled) import("./renderer-CpAcx8Ar.js").then(({ ReactGrabRenderer }) => {
			if (disposed) return;
