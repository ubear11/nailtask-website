 ...
 */
import { $ as MODIFIER_KEYS, A as Z, B as ARROW_KEYS, D as Oe, E as Me, F as nativeRequestAnimationFrame, H as DEFAULT_ACTION_ID, I as waitUntilNextFrame, L as hideFromThirdParties, M as we, N as d, O as Se, P as nativeCancelAnimationFrame, Q as MAX_SESSION_STORAGE_SIZE_BYTES, R as detectCspNonce, S as getNearestComponentName, T as resolveSource, W as DRAG_SELECTION_COVERAGE_THRESHOLD, _ as isRootElement, _t as Z_INDEX_OVERLAY, a as freezeGlobalAnimations, b as getElementContext, c as clearElementPositionCache, d as freezePseudoStates, et as MOUNT_ROOT_RECHECK_DELAY_MS, f as resumePointerEventsFreeze, ft as TOOLBAR_DEFAULT_POSITION_RATIO, g as isValidGrabbableElement, gt as ZOOM_DETECTION_THRESHOLD, h as clearVisibilityCache, ht as VERSION, i as freezeAnimations, j as ve, l as getElementAtPosition, m as unfreezePseudoStates, n as logRecoverableError, nt as OFFSCREEN_POSITION, o as unfreezeGlobalAnimations, p as suspendPointerEventsFreeze, q as FEEDBACK_DURATION_MS, r as freezeAllAnimations, s as openFile, t as freezeUpdates, tt as NEXTJS_REVALIDATION_DELAY_MS, u as getElementsAtPoint, v as checkIsNextProject, w as getStackContext, y as getComponentDisplayName, z as getTagName } from "./freeze-updates-GAYNGHjS.js";
import { a as isMac, g as isElementConnected, h as isKeyboardEventTriggeredByInput, i as isEventFromOverlay, m as hasTextSelectionOnPage, n as saveToolbarState, p as hasTextSelectionInInput, r as resolveActionEnabled, s as clampToRange, t as loadToolbarState } from "./state-DGP1F5Fj.js";
import { n as extractElementCss, r as createElementSelector, t as disposeBaselineStyles } from "./extract-element-css-BkXjQhL1.js";
...
const getRenderedBy = (element) => {
	...
	let compositeFiber = fiber.return;
	while (compositeFiber) {
		if (ve(compositeFiber)) {
			const name = we(compositeFiber.type);
			if (name) {
				ownerName = name;
				break;
			}
...
const initRenderTimeline = () => {
	...
	isInitialized = true;
	Me({ onCommitFiberRoot(_rendererID, root) {
		const rootFiber = root.current;
		if (!rootFiber) return;
		const { totalTime } = Se(rootFiber);
		const componentTypes = /* @__PURE__ */ new Set();
		Oe(root, (fiber) => {
			if (ve(fiber) && we(fiber.type)) componentTypes.add(fiber.type);
		});
...
const getComponentType = (element) => {
	...
	let compositeFiber = fiber.return;
	while (compositeFiber) {
		if (ve(compositeFiber)) return compositeFiber.type;
		compositeFiber = compositeFiber.return;
	}
	return null;
};
...
const init = (rawOptions) => {
	...
	return createRoot((dispose) => {
		...
		const inspectPropertiesState = createMemo(() => {
		...
		if (pluginRegistry.store.theme.enabled) import("./renderer-ohbxzY8_.js").then(({ ReactGrabRenderer }) => {
			if (disposed) return;
