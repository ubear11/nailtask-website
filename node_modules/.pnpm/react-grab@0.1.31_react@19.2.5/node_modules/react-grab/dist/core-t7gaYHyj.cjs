...
const require_extract_element_css = require('./extract-element-css-EnvrIHtf.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-Cdh6G3WO.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
