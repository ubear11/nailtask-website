...
//#endregion
//#region src/utils/extract-i18n-labels.ts
const I18N_COMPONENT_PATTERNS = [{
	displayNames: ["Trans"],
	keyProp: "i18nKey",
	namespaceProp: "ns",
	defaultMessageProp: "defaults"
}, {
	displayNames: ["FormattedMessage", "FormattedHTMLMessage"],
	keyProp: "id",
	defaultMessageProp: "defaultMessage"
}];
const findMatchingPattern = (displayName) => I18N_COMPONENT_PATTERNS.find((pattern) => pattern.displayNames.some((name) => displayName === name));
const extractLabelFromFiber = (fiber, pattern) => {
	const props = fiber.memoizedProps;
	if (!props) return null;
	const translationKey = props[pattern.keyProp];
	if (typeof translationKey !== "string" || translationKey.length === 0) return null;
	const label = { translationKey };
	if (pattern.namespaceProp) {
		const namespace = props[pattern.namespaceProp];
		if (typeof namespace === "string" && namespace.length > 0) label.namespace = namespace;
	}
	if (pattern.defaultMessageProp) {
		const defaultMessage = props[pattern.defaultMessageProp];
		if (typeof defaultMessage === "string" && defaultMessage.length > 0) label.defaultMessage = defaultMessage;
	}
	return label;
};
const extractI18nLabels = (element) => {
	if (!Te$1()) return [];
	...
	j$1(fiber, (currentFiber) => {
		...
		if (!ve$1(currentFiber)) return false;
		const displayName = we$1(currentFiber.type);
		if (!displayName) return false;
		const pattern = findMatchingPattern(displayName);
		if (!pattern) return false;
		const label = extractLabelFromFiber(currentFiber, pattern);
		if (!label || seenKeys.has(label.translationKey)) return false;
		seenKeys.add(label.translationKey);
		labels.push(label);
		return false;
	}, false);
	return labels;
};
const formatI18nLabels = (labels) => {
	if (labels.length === 0) return "";
	return labels.map((label) => {
		let entry = `"${label.translationKey}"`;
		if (label.namespace) entry += ` (ns: ${label.namespace})`;
		if (label.defaultMessage) entry += ` (default: "${label.defaultMessage}")`;
		return `\n  i18n: ${entry}`;
	}).join("");
};
...
const getElementContext = async (element, options = {}) => {
	...
	const i18nLabels = formatI18nLabels(extractI18nLabels(resolvedElement));
	if (stackContext) return `${html}${stackContext}${i18nLabels}`;
	return `${getFallbackContext(resolvedElement)}${i18nLabels}`;
};
...
//#endregion
export { OVERLAY_BORDER_COLOR_DEFAULT as $, nativeRequestAnimationFrame as A, DROPDOWN_EDGE_TRANSFORM_ORIGIN as B, getStack as C, Te$1 as D, extractI18nLabels as E, ARROW_KEYS as F, LERP_CONVERGENCE_THRESHOLD_PX as G, FEEDBACK_DURATION_MS as H, ARROW_MAX_LABEL_WIDTH_RATIO as I, MODIFIER_KEYS as J, MAX_SELECTOR_COMBINATIONS as K, DEFAULT_ACTION_ID as L, hideFromThirdParties as M, detectCspNonce as N, Z$1 as O, getTagName as P, OPACITY_CONVERGENCE_THRESHOLD as Q, DRAG_LERP_FACTOR as R, getNearestComponentName as S, resolveSource as T, FROZEN_GLOW_COLOR as U, DROPDOWN_OFFSCREEN_POSITION as V, HINT_FLIP_IN_ANIMATION as W, NEXTJS_REVALIDATION_DELAY_MS as X, MOUNT_ROOT_RECHECK_DELAY_MS as Y, OFFSCREEN_POSITION as Z, isRootElement as _, freezeGlobalAnimations as a, RELEVANT_CSS_PROPERTIES as at, getElementContext as b, clearElementPositionCache as c, SELECTION_LERP_FACTOR as ct, freezePseudoStates as d, TOOLTIP_BASE_CLASS as dt, OVERLAY_BORDER_COLOR_DRAG as et, resumePointerEventsFreeze as f, VERSION as ft, isValidGrabbableElement as g, supportsDisplayP3 as gt, clearVisibilityCache as h, Z_INDEX_OVERLAY_CANVAS as ht, freezeAnimations as i, OVERLAY_FILL_COLOR_INSPECT as it, waitUntilNextFrame as j, nativeCancelAnimationFrame as k, getElementAtPosition as l, TOOLBAR_DEFAULT_POSITION_RATIO as lt, unfreezePseudoStates as m, Z_INDEX_OVERLAY as mt, logRecoverableError as n, OVERLAY_FILL_COLOR_DEFAULT as nt, unfreezeGlobalAnimations as o, SELECTION_HINT_CYCLE_INTERVAL_MS as ot, suspendPointerEventsFreeze as p, ZOOM_DETECTION_THRESHOLD as pt, MAX_SESSION_STORAGE_SIZE_BYTES as q, freezeAllAnimations as r, OVERLAY_FILL_COLOR_DRAG as rt, openFile as s, SELECTION_LABEL_OFFSCREEN_PX as st, freezeUpdates as t, OVERLAY_BORDER_COLOR_INSPECT as tt, getElementsAtPoint as u, TOOLBAR_SHAKE_TOOLTIP_DURATION_MS as ut, checkIsNextProject as v, getStackContext as w, getHTMLPreview as x, getComponentDisplayName as y, DRAG_SELECTION_COVERAGE_THRESHOLD as z };