...
const require_extract_element_css = require('./extract-element-css-CHjjIquQ.cjs');
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CQiL1VS0.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
