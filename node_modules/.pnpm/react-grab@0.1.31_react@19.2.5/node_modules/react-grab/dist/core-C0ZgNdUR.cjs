...
const require_extract_element_css = require('./extract-element-css-D6UwJjf4.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CS6q5sh-.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
