...
const require_extract_element_css = require('./extract-element-css-BTzDGIs3.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DWZQjHeF.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
