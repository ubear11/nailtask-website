...
const require_extract_element_css = require('./extract-element-css-vTF0v8xb.cjs');
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DF7p6EUi.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
