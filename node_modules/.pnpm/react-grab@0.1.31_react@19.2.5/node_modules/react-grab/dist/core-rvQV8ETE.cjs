...
const require_extract_element_css = require('./extract-element-css-_iZ2Cmsh.cjs');
...
const getAncestorElements = (element) => {
	const ancestors = [];
	let current = element.parentElement;
	while (current && !require_freeze_updates.isRootElement(current)) {
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		const inspectBounds = (0, solid_js.createMemo)(() => {
		...
		const inspectAncestorElements = (0, solid_js.createMemo)(() => {
			if (!isInspectMode()) return [];
			const element = effectiveElement();
			if (!element) return [];
			return [...getAncestorElements(element).reverse(), element];
		});
		const inspectNavigationItems = (0, solid_js.createMemo)(() => inspectAncestorElements().map((element) => ({
			tagName: require_freeze_updates.getTagName(element) || "element",
			componentName: require_freeze_updates.getComponentDisplayName(element) ?? void 0
		})));
		const [inspectActiveIndex, setInspectActiveIndex] = (0, solid_js.createSignal)(-1);
		(0, solid_js.createEffect)((0, solid_js.on)(inspectAncestorElements, (elements) => {
			setInspectActiveIndex(elements.length - 1);
		}));
		const inspectNavigationState = (0, solid_js.createMemo)(() => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BlpyWzn4.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
			disposeRenderer = (0, solid_js_web.render)(() => {
				return (0, solid_js_web.createComponent)(ReactGrabRenderer, {
					...
					get inspectVisible() {
						return (0, solid_js_web.memo)(() => !!isInspectMode())() && inspectBounds().length > 0;
					},
					get inspectBounds() {
