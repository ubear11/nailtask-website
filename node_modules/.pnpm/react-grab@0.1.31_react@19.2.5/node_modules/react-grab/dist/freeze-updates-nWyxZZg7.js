...
const RENDER_DURATION_NEGLIGIBLE_MS = .1;
...
let I$1 = 0;
const L$1 = /* @__PURE__ */ new WeakMap(), R$1 = (e, t = I$1++) => {
	L$1.set(e, t);
}, z$1 = (e) => {
	let t = L$1.get(e);
	return !t && e.alternate && (t = L$1.get(e.alternate)), t || (t = I$1++, R$1(e, t)), t;
}, B$1 = (e, t, n) => {
	let r = t;
	for (; r != null;) {
		if (L$1.has(r) || z$1(r), !O$1(r) && w$1(r) && e(r, `mount`), r.tag === 13) if (r.memoizedState !== null) {
			...
			let t = null;
			r.child !== null && (t = r.child.child), t !== null && B$1(e, t, !1);
		}
		else r.child != null && B$1(e, r.child, !0);
		r = n ? r.sibling : null;
	}
}, V$1 = (e, t, n, r) => {
	if (L$1.has(t) || z$1(t), !n) return;
	L$1.has(n) || z$1(n);
	let i = t.tag === 13, a = !O$1(t);
	a && w$1(t) && e(t, `update`);
	let o = i && n.memoizedState !== null, s = i && t.memoizedState !== null;
	if (o && s) {
		let r = t.child?.sibling ?? null, i = n.child?.sibling ?? null;
		r !== null && i !== null && V$1(e, r, i, t);
	} else if (o && !s) {
		let n = t.child;
		n !== null && B$1(e, n, !0);
	} else if (!o && s) {
	...
	(t.tag === 3 || !O$1(t)) && e(t, `unmount`);
}, U$1 = (e, t) => {
	let n = t.tag === 13 && t.memoizedState !== null, r = t.child;
	for (n && (r = (t.child?.sibling ?? null)?.child ?? null); r !== null;) r.return !== null && (H$1(e, r), U$1(e, r)), r = r.sibling;
};
let De$1 = 0;
const W$1 = /* @__PURE__ */ new WeakMap(), Oe$1 = (e, t) => {
	let n = `current` in e ? e.current : e, r = W$1.get(e);
	r || (r = {
		id: De$1++,
		prevFiber: null
	}, W$1.set(e, r));
	...
	if (!n) H$1(t, n);
		...
		!e && r ? B$1(t, n, !1) : e && r ? V$1(t, n, n.alternate, null) : e && !r && H$1(t, n);
	} else B$1(t, n, !0);
	r.prevFiber = n;
};
...
const J$1 = () => {
	...
	return null;
}, Q$1 = Error(), $$1 = /* @__PURE__ */ new Set(), Ne$1 = (e, n = {}) => {
...
const ft = (e, t, n) => {
	let r = t[n].source;
	hookSearch: for (let i = 0; i < e.length; i++) if (e[i].source === r) {
		for (let r = n + 1, a = i + 1; r < t.length && a < e.length; r++, a++) if (e[a].source !== t[r].source) continue hookSearch;
		return i;
	}
	return -1;
}, pt = (e, t) => {
	let n = ft(t, e, dt);
	if (n !== -1) return n;
	for (let r = 0; r < e.length && r < 5; r++) if (n = ft(t, e, r), n !== -1) return dt = r, n;
	return -1;
}, $ = (e) => {
	if (!e) return ``;
	let t = e.lastIndexOf(`[as `);
	if (t !== -1) return $(e.slice(t + 4, -1));
	if (t = e.lastIndexOf(`.`), t = t === -1 ? 0 : t + 1, e.slice(t).startsWith(`unstable_`) && (t += 9), e.slice(t).startsWith(`experimental_`) && (t += 13), e.slice(t, t + 3) === `use`) {
		if (e.length - t === 3) return `Use`;
		t += 3;
	}
	return e.slice(t);
}, mt = (e, t) => {
	let n = $(e);
	return t === `HostTransitionStatus` ? n === t || n === `FormStatus` : n === t;
}, ht = (e, t) => {
	let n = ut().get(t.primitive);
	if (n === void 0) return -1;
	for (let r = 0; r < n.length && r < e.length; r++) if (n[r].source !== e[r].source) return r < e.length - 1 && mt(e[r].functionName, t.dispatcherHookName) && r++, r < e.length - 1 && mt(e[r].functionName, t.dispatcherHookName) && r++, r;
	return -1;
}, gt = (e, t) => {
	let n = J(t.stackError), r = pt(e, n), i = ht(n, t);
	return r === -1 || i === -1 || r - i < 2 ? i === -1 ? [null, null] : [n[i - 1] ?? null, null] : [n[i - 1] ?? null, n.slice(i, r - 1)];
}, _t = new Set([
	`Context`,
	`Context (use)`,
	`DebugValue`,
	...
	let n = [], r = null, i = n, a = 0, o = [];
	for (let s of t) {
		let [t, c] = gt(e, s), l = s.displayName;
		if (l === null && t !== null && (l = $(t.functionName) || $(s.dispatcherHookName)), c !== null) {
			let e = 0;
			if (r !== null) {
				for (; e < c.length && e < r.length && c[c.length - e - 1].source === r[r.length - e - 1].source;) e++;
				for (let t = r.length - 1; t > e; t--) i = o.pop() ?? n;
			}
			for (let t = c.length - e - 1; t >= 1; t--) {
				let e = [], n = c[t], r = {
					id: null,
					isStateEditable: !1,
					name: $(c[t - 1].functionName),
					value: void 0,
					subHooks: e,
					hookSource: {
				...
				i.push(r), o.push(i), i = e;
			}
			r = c;
		}
		let { primitive: u } = s, d = _t.has(u) ? null : a++, f = u === `Reducer` || u === `State`, p = l || u, m = c?.[0], h = {
			lineNumber: m?.lineNumber ?? null,
			columnNumber: m?.columnNumber ?? null,
			functionName: m?.functionName ?? null,
	...
	for (let t = 0; t < e.length; t++) {
		let r = e[t];
		r.name === `DebugValue` && r.subHooks.length === 0 ? (e.splice(t, 1), t--, n.push(r)) : yt(r.subHooks, r);
	}
	t !== null && (n.length === 1 ? t.value = n[0].value : n.length > 1 && (t.value = n.map(({ value: e }) => e)));
}, bt = (e, t) => {
	let n = t;
	for (; n;) {
		if (n.tag === 10) {
			let t = n.type;
			`_context` in t && t._context !== void 0 && (t = t._context), e.has(t) || (e.set(t, t._currentValue), t._currentValue = n.memoizedProps.value);
		}
		n = n.return;
	}
}, xt = (e) => {
	...
	if (e === q) return;
	if (e instanceof Error && e.name === `ReactDebugToolsUnsupportedHookError`) throw e;
	let t = Error(`Error rendering inspected component`, { cause: e });
	throw t.name = `ReactDebugToolsRenderError`, t.cause = e, t;
}, Ct = (e, t) => {
	if (e && typeof e == `object` && `defaultProps` in e && e.defaultProps) {
		let n = { ...t }, r = e.defaultProps;
	...
	return vt(i === void 0 ? [] : J(i), r);
}, Dt = () => {
	let e = Re();
	if (!e) throw Error(`No React renderer found. Make sure React is loaded and bippy's hook is installed.`);
	return e;
}, Ot = (e) => {
	if (Object.prototype.hasOwnProperty.call(e, `dependencies`)) {
		let t = e.dependencies;
		W = t === null ? null : t.firstContext;
	} else if (Object.prototype.hasOwnProperty.call(e, `dependencies_old`)) {
		let t = e.dependencies_old;
		W = t === null ? null : t.firstContext;
	} else if (Object.prototype.hasOwnProperty.call(e, `dependencies_new`)) {
		let t = e.dependencies_new;
		W = t === null ? null : t.firstContext;
	} else if (Object.prototype.hasOwnProperty.call(e, `contextDependencies`)) {
		let t = e.contextDependencies;
		W = t === null ? null : t.first;
	} else throw Error(`Unsupported React version.`);
}, kt = (e) => {
	let t = Dt();
	if (e.tag !== 0 && e.tag !== 15 && e.tag !== 11) throw Error(`Unknown Fiber. Needs to be a function component to inspect hooks.`);
	ut(), U = e.memoizedState, H = e;
	let n = e.dependencies && e.dependencies._debugThenableState, r = n ? n.thenables || n : null;
	K = Array.isArray(r) ? r : null, G = 0, Ot(e);
	let i = e.type, a = e.memoizedProps;
	i !== e.elementType && (a = Ct(i, a));
	let o = wt(), s = /* @__PURE__ */ new Map();
	try {
		return W !== null && !Object.prototype.hasOwnProperty.call(W, `memoizedValue`) && bt(s, e), e.tag === 11 ? Et(t, () => {
			i.render(a, e.ref);
		}) : Et(t, () => {
			i(a);
		});
	} finally {
		H = null, U = null, W = null, K = null, G = 0, xt(s), Tt(o);
	}
}, At = new Set([
	`Effect`,
	`LayoutEffect`,
	...
	`ImperativeHandle`,
	`DebugValue`
]), jt = /(?:const|let|var)\s+((?:\[[\s\S]*?\]|\w+))\s*=\s*(?:[\w$.]+\.)*use[A-Z]\w*\s*(?:<[\s\S]*?>)?\s*\(/g, Mt = (e) => `${e.fileName ?? ``}:${e.lineNumber ?? 0}:${e.columnNumber ?? 0}`, Nt = (e) => {
	let t = [], n = (e) => {
		for (let r of e) At.has(r.name) || (t.push(r), r.subHooks.length > 0 && n(r.subHooks));
	};
	return n(e), t;
}, Pt = (e, t, n) => {
	if (!t) return null;
	let r = e.indexOf(n);
	return r === -1 ? null : t[r] ?? null;
}, Ft = (e, t) => {
	let n = Pt(e.sources, e.sourcesContent, t);
	if (n) return n;
	if (e.sections) for (let n of e.sections) {
		let e = Pt(n.map.sources, n.map.sourcesContent, t);
		if (e) return e;
	}
	return null;
}, It = (e) => {
...
`), i = t - 1;
	...
	let a = Math.max(0, i - 10), o = r.slice(a, i + 1).join(`
`), s = [...o.matchAll(jt)], c = o.lastIndexOf(`
`) + 1 + n, l = s.filter((e) => e.index <= c).at(-1);
	return l ? It(l[1]) : null;
}, Rt = async (e, t, n, r) => {
	let { sourceMapsByFile: i, sourceContentCache: a, fetchFn: o } = r;
	i.has(e) || i.set(e, await O(e, !0, o));
	let s = i.get(e) ?? null;
	if (s) {
		let r = T(s, t, n);
		if (r?.fileName && r.lineNumber !== void 0) {
			let t = `sourcemap:${e}:${r.fileName}`;
			a.has(t) || a.set(t, Ft(s, r.fileName));
			let n = a.get(t) ?? null;
			if (n) return {
				sourceCode: n,
	...
	if (!a.has(e)) try {
		let t = await (o ?? fetch)(e);
		a.set(e, t.ok ? await t.text() : null);
	} catch {
		a.set(e, null);
	}
	let c = a.get(e) ?? null;
	...
	let n = /* @__PURE__ */ new Map(), r = Nt(e);
	if (r.length === 0) return n;
	let i = {
		sourceMapsByFile: /* @__PURE__ */ new Map(),
		sourceContentCache: /* @__PURE__ */ new Map(),
		fetchFn: t
	};
	return await Promise.all(r.map(async (e) => {
		let t = e.hookSource;
		if (!t || !t.fileName || t.lineNumber === null || t.columnNumber === null) return;
		let r = await Rt(t.fileName, t.lineNumber, t.columnNumber, i);
		if (!r) return;
		let a = Lt(r.sourceCode, r.lineNumber, r.columnNumber);
		if (a) {
			let e = Mt(t);
			n.set(e, a);
		}
...
const resolveSource = async (element) => {
	...
	return {
		filePath: Me(resolvedFrame.fileName),
...
//#endregion
export { MODIFIER_KEYS as $, Te$1 as A, ARROW_KEYS as B, getStack as C, Me$1 as D, kt as E, nativeRequestAnimationFrame as F, DROPDOWN_EDGE_TRANSFORM_ORIGIN as G, DEFAULT_ACTION_ID as H, waitUntilNextFrame as I, FROZEN_GLOW_COLOR as J, DROPDOWN_OFFSCREEN_POSITION as K, hideFromThirdParties as L, ve$1 as M, we$1 as N, Oe$1 as O, nativeCancelAnimationFrame as P, MAX_SESSION_STORAGE_SIZE_BYTES as Q, detectCspNonce as R, getNearestComponentName as S, resolveSource as T, DRAG_LERP_FACTOR as U, ARROW_MAX_LABEL_WIDTH_RATIO as V, DRAG_SELECTION_COVERAGE_THRESHOLD as W, LERP_CONVERGENCE_THRESHOLD_PX as X, HINT_FLIP_IN_ANIMATION as Y, MAX_SELECTOR_COMBINATIONS as Z, isRootElement as _, ZOOM_DETECTION_THRESHOLD as _t, freezeGlobalAnimations as a, OVERLAY_BORDER_COLOR_DRAG as at, getElementContext as b, supportsDisplayP3 as bt, clearElementPositionCache as c, RELEVANT_CSS_PROPERTIES as ct, freezePseudoStates as d, SELECTION_LABEL_OFFSCREEN_PX as dt, MOUNT_ROOT_RECHECK_DELAY_MS as et, resumePointerEventsFreeze as f, SELECTION_LERP_FACTOR as ft, isValidGrabbableElement as g, VERSION as gt, clearVisibilityCache as h, TOOLTIP_BASE_CLASS as ht, freezeAnimations as i, OVERLAY_BORDER_COLOR_DEFAULT as it, Z$1 as j, Se$1 as k, getElementAtPosition as l, RENDER_DURATION_NEGLIGIBLE_MS as lt, unfreezePseudoStates as m, TOOLBAR_SHAKE_TOOLTIP_DURATION_MS as mt, logRecoverableError as n, OFFSCREEN_POSITION as nt, unfreezeGlobalAnimations as o, OVERLAY_FILL_COLOR_DEFAULT as ot, suspendPointerEventsFreeze as p, TOOLBAR_DEFAULT_POSITION_RATIO as pt, FEEDBACK_DURATION_MS as q, freezeAllAnimations as r, OPACITY_CONVERGENCE_THRESHOLD as rt, openFile as s, OVERLAY_FILL_COLOR_DRAG as st, freezeUpdates as t, NEXTJS_REVALIDATION_DELAY_MS as tt, getElementsAtPoint as u, SELECTION_HINT_CYCLE_INTERVAL_MS as ut, checkIsNextProject as v, Z_INDEX_OVERLAY as vt, getStackContext as w, getHTMLPreview as x, getComponentDisplayName as y, Z_INDEX_OVERLAY_CANVAS as yt, getTagName as z };