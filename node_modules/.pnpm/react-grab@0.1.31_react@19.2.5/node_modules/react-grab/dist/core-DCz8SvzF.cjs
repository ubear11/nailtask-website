...
const require_extract_element_css = require('./extract-element-css-Or47PM6N.cjs');
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		const inspectPropertiesState = (0, solid_js.createMemo)(() => {
			...
			const source = Boolean(fileName) && /\.(tsx?|jsx?)$/i.test(fileName) && !/[_]{2,}|[0-9a-f]{8,}/i.test(fileName) ? `${fileName}${lineNumber ? `:${lineNumber}` : ""}` : "";
			const reactProps = getReactProps(element);
			const hooks = getHooksState(element);
			const timeline = getTimelineForElement(element);
			if (!(source || reactProps.length > 0 || hooks.length > 0 || timeline && timeline.totalRenderCount > 0)) return void 0;
			return {
				reactProps,
				hooks,
				source,
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-3AxW2bw2.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
