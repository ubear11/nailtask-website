...
const require_extract_element_css = require('./extract-element-css-_aFPl5y4.cjs');
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-TkTQ8ZzV.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
