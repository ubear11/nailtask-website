...
const require_extract_element_css = require('./extract-element-css--48dnvZQ.cjs');
...
const getHooksState = (element) => {
	...
	while (hookState && hookIndex < hookTypes.length) {
		...
		if (STATEFUL_HOOK_TYPES.has(hookType)) {
			rows.push({
				label: hookType,
				value: formatPropValue(hookState.memoizedState)
			});
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-odhUN60N.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
