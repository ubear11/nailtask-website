...
const require_extract_element_css = require('./extract-element-css-Bi1y8V4z.cjs');
...
const removeCommentItem = (itemId) => persistCommentItems(commentItems.filter((innerItem) => innerItem.id !== itemId));
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-tLg9bC5E.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
