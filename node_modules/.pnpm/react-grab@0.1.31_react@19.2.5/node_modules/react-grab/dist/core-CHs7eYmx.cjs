...
const require_extract_element_css = require('./extract-element-css-B60cn1iV.cjs');
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DNaq5DWp.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
