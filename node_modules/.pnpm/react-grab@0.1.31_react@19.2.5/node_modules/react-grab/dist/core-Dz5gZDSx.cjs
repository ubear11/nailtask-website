...
const require_extract_element_css = require('./extract-element-css-B_kkksXU.cjs');
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-CZkpk79m.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
