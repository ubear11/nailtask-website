...
const require_extract_element_css = require('./extract-element-css-Sl4axlhl.cjs');
...
//#endregion
//#region ../../node_modules/.pnpm/solid-js@1.9.10/node_modules/solid-js/store/dist/server.js
...
const init = (rawOptions) => {
	...
	return require_state.createRoot((dispose) => {
		...
		const isHoldingKeys = require_state.createMemo(() => store.current.state === "holding");
		const isActivated = require_state.createMemo(() => store.current.state === "active");
		const isFrozenPhase = require_state.createMemo(() => store.current.state === "active" && store.current.phase === "frozen");
		const isDragging = require_state.createMemo(() => store.current.state === "active" && store.current.phase === "dragging");
		const didJustDrag = require_state.createMemo(() => store.current.state === "active" && store.current.phase === "justDragged");
		const isCopying = require_state.createMemo(() => store.current.state === "copying");
		const didJustCopy = require_state.createMemo(() => store.current.state === "justCopied");
		const isPromptMode = require_state.createMemo(() => store.current.state === "active" && Boolean(store.current.isPromptMode));
		const isCommentMode = require_state.createMemo(() => store.pendingCommentMode || isPromptMode());
		const isPendingDismiss = require_state.createMemo(() => store.current.state === "active" && Boolean(store.current.isPromptMode) && Boolean(store.current.isPendingDismiss));
		...
		const [isEnabled, setIsEnabled] = require_state.createSignal(savedToolbarState?.enabled ?? true);
		...
		const [commentsDropdownPosition, setCommentsDropdownPosition] = require_state.createSignal(null);
		const [toolbarMenuPosition, setToolbarMenuPosition] = require_state.createSignal(null);
		const [clearPromptPosition, setClearPromptPosition] = require_state.createSignal(null);
		let toolbarElement;
		let dropdownTrackingFrameId = null;
		const commentElementMap = /* @__PURE__ */ new Map();
		const [clockFlashTrigger, setClockFlashTrigger] = require_state.createSignal(0);
		const [isCommentsHoverOpen, setIsCommentsHoverOpen] = require_state.createSignal(false);
		...
		const commentsDisconnectedItemIds = require_state.createMemo(() => {
		...
		const [isInspectMode, setIsInspectMode] = require_state.createSignal(false);
		...
		const [actionCycleItems, setActionCycleItems] = require_state.createSignal([]);
		const [actionCycleActiveIndex, setActionCycleActiveIndex] = require_state.createSignal(null);
		...
		const inspectBounds = require_state.createMemo(() => {
		...
		const inspectAncestorElements = require_state.createMemo(() => {
			if (!isInspectMode()) return [];
			const element = effectiveElement();
			if (!element) return [];
			return [...getAncestorElements(element).reverse(), element];
		});
		const inspectNavigationItems = require_state.createMemo(() => inspectAncestorElements().map((element) => ({
			tagName: require_freeze_updates.getTagName(element) || "element",
			componentName: require_freeze_updates.getComponentDisplayName(element) ?? void 0
		})));
		const [inspectActiveIndex, setInspectActiveIndex] = require_state.createSignal(-1);
		require_state.createEffect(require_state.on(inspectAncestorElements, (elements) => {
			setInspectActiveIndex(elements.length - 1);
		}));
		const inspectNavigationState = require_state.createMemo(() => {
		...
		require_state.createEffect(require_state.on(selectionElement, () => {
			resetActionCycle();
		}));
		require_state.createEffect(require_state.on(canCycleActions, (isEnabled) => {
		...
		const availableActionCycleItems = require_state.createMemo(() => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-JOJiAa_9.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
			disposeRenderer = require_state.render(() => {
				return require_state.createComponent(ReactGrabRenderer, {
					...
					get inspectVisible() {
						return require_state.memo(() => !!isInspectMode())() && inspectBounds().length > 0;
					},
					get inspectBounds() {
					...
					get mouseX() {
						return require_state.memo(() => store.frozenElements.length > 1)() ? void 0 : cursorPosition().x;
					},
					get isFrozen() {
					...
					get isCommentsPinned() {
						return require_state.memo(() => commentsDropdownPosition() !== null)() && !isCommentsHoverOpen();
					},
