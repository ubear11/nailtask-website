...
const buildHookRows = (hooksTree, hookNames) => hooksTree.filter((hookNode) => hookNode.isStateEditable).slice(0, 8).map((hookNode) => {
	return {
...
const getHooksState = (element) => {
	...
	return buildHookRows(require_freeze_updates.kt(compositeFiber));
};
const resolveHookNames = async (element) => {
	const compositeFiber = findNearestCompositeFiber(element);
	if (!compositeFiber) return null;
	const hooksTree = require_freeze_updates.kt(compositeFiber);
