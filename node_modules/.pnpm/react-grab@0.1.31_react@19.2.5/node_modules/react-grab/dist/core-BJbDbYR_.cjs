...
const require_extract_element_css = require('./extract-element-css-AXEuvJKz.cjs');
...
const createGrabStore = (input) => {
	const [store, setStore] = (0, solid_js_store.createStore)(createInitialStore(input));
	const clearFrozenElement = () => {
		setStore((0, solid_js_store.produce)((draft) => {
	...
	const setActivePhase = (phase) => {
		setStore("current", (0, solid_js_store.produce)((current) => {
	...
	const actions = {
		...
		enterPromptMode: (position, element) => {
			...
			} else setStore("current", (0, solid_js_store.produce)((current) => {
		...
		exitPromptMode: () => {
			if (store.current.state === "active") setStore("current", (0, solid_js_store.produce)((current) => {
				if (current.state === "active") {
		...
		updateLabelInstance: (instanceId, status, errorMessage) => {
			...
			if (index !== -1) setStore("labelInstances", index, (0, solid_js_store.produce)((instance) => {
...
//#endregion
//#region src/utils/format-prop-value.ts
...
const formatPropValue = (value) => {
	...
	if (formatted.length > 40) return `${formatted.slice(0, 40 - 1)}…`;
	return formatted;
};

//#endregion
//#region src/utils/find-nearest-composite-fiber.ts
const findNearestCompositeFiber = (element) => {
	const hostFiber = require_freeze_updates.Z(element);
	if (!hostFiber) return null;
	let currentFiber = hostFiber.return;
	while (currentFiber) {
		if (require_freeze_updates.ve(currentFiber)) return currentFiber;
...
//#endregion
//#region src/utils/get-react-props.ts
...
const getReactProps = (element) => {
	...
	return Object.entries(fiberProps).filter(([propKey]) => !HIDDEN_PROP_KEYS.has(propKey) && !propKey.startsWith("__")).slice(0, 6).map(([propKey, propValue]) => ({
		label: propKey,
		value: formatPropValue(propValue)
	}));
};

//#endregion
//#region src/utils/get-hooks-state.ts
const collectEditableHooks = (nodes) => nodes.flatMap((hookNode) => hookNode.isStateEditable ? [hookNode] : collectEditableHooks(hookNode.subHooks));
const buildHookRows = (hooksTree, hookNames) => collectEditableHooks(hooksTree).slice(0, 8).map((hookNode) => {
	return {
		label: (hookNode.hookSource && hookNames ? hookNames.get(require_freeze_updates.Mt(hookNode.hookSource)) : void 0) ?? hookNode.name,
		value: formatPropValue(hookNode.value)
	};
});
const safeGetFiberHooks = (fiber) => {
	try {
		return require_freeze_updates.kt(fiber);
	} catch {
		return [];
	}
};
const getHooksState = (element) => {
	const compositeFiber = findNearestCompositeFiber(element);
	if (!compositeFiber) return [];
	return buildHookRows(safeGetFiberHooks(compositeFiber));
};
const resolveHookNames = async (element) => {
	const compositeFiber = findNearestCompositeFiber(element);
	if (!compositeFiber) return null;
	const hooksTree = safeGetFiberHooks(compositeFiber);
	if (hooksTree.length === 0) return null;
	const hookNames = await require_freeze_updates.zt(hooksTree);
	if (hookNames.size === 0) return null;
	return buildHookRows(hooksTree, hookNames);
};

//#endregion
//#region src/utils/render-timeline.ts
const commitBuffer = [];
const [commitVersion, setCommitVersion] = (0, solid_js.createSignal)(0);
let isInitialized = false;
const initRenderTimeline = () => {
	if (isInitialized) return;
	isInitialized = true;
	require_freeze_updates.Me({ onCommitFiberRoot: (_rendererID, root) => {
		const fiberDurations = /* @__PURE__ */ new Map();
		require_freeze_updates.Oe(root, (fiber) => {
			if (require_freeze_updates.ve(fiber) && require_freeze_updates.we(fiber.type)) {
				const fiberId = require_freeze_updates.z(fiber);
				const { selfTime } = require_freeze_updates.Se(fiber);
		...
		if (commitBuffer.length > 100) commitBuffer.splice(0, commitBuffer.length - 100);
		setCommitVersion((previousVersion) => previousVersion + 1);
	} });
};
const getTimelineForElement = (element) => {
	commitVersion();
	const compositeFiber = findNearestCompositeFiber(element);
	if (!compositeFiber) return void 0;
	const inspectedFiberId = require_freeze_updates.z(compositeFiber);
	const commits = commitBuffer.filter((entry) => entry.fiberDurations.has(inspectedFiberId)).map((entry) => ({
...
//#endregion
//#region src/utils/format-source-location.ts
const SOURCE_FILE_EXTENSION_PATTERN = /\.(tsx?|jsx?)$/i;
const BUNDLED_FILENAME_PATTERN = /[_]{2,}|[0-9a-f]{8,}/i;
const formatSourceLocation = (filePath, lineNumber) => {
	if (!filePath) return "";
	const fileName = filePath.split(/[/\\]/).pop() ?? "";
	if (!fileName) return "";
	if (!SOURCE_FILE_EXTENSION_PATTERN.test(fileName) || BUNDLED_FILENAME_PATTERN.test(fileName)) return "";
...
const createPluginRegistry = (initialOptions = {}) => {
	...
	const [store, setStore] = (0, solid_js_store.createStore)({
		theme: DEFAULT_THEME,
		options: {
			...DEFAULT_OPTIONS,
			...initialOptions
...
const copyStylesPlugin = createPendingSelectionPlugin({
	...
	contextMenuAction: (api) => ({
		...
		onAction: async (context) => {
			await context.performWithFeedback(async () => {
				return copyContent(appendStackContext(context.elements.map(require_extract_element_css.extractElementCss).join("\n\n"), await api.getStackContext(context.element)), {
					componentName: context.componentName,
					tagName: context.tagName
				});
			});
	...
	cleanup: require_extract_element_css.disposeBaselineStyles
});
...
const confirmClear = () => {
	...
	try {
		...
		require_freeze_updates.logRecoverableError("Failed to save clear preference to sessionStorage", error);
	}
};
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		let disposed = false;
		let disposeRenderer;
		...
		const isHoldingKeys = (0, solid_js.createMemo)(() => store.current.state === "holding");
		const isActivated = (0, solid_js.createMemo)(() => store.current.state === "active");
		const isFrozenPhase = (0, solid_js.createMemo)(() => store.current.state === "active" && store.current.phase === "frozen");
		const isDragging = (0, solid_js.createMemo)(() => store.current.state === "active" && store.current.phase === "dragging");
		const didJustDrag = (0, solid_js.createMemo)(() => store.current.state === "active" && store.current.phase === "justDragged");
		const isCopying = (0, solid_js.createMemo)(() => store.current.state === "copying");
		const didJustCopy = (0, solid_js.createMemo)(() => store.current.state === "justCopied");
		const isPromptMode = (0, solid_js.createMemo)(() => store.current.state === "active" && Boolean(store.current.isPromptMode));
		const isCommentMode = (0, solid_js.createMemo)(() => store.pendingCommentMode || isPromptMode());
		const isPendingDismiss = (0, solid_js.createMemo)(() => store.current.state === "active" && Boolean(store.current.isPromptMode) && Boolean(store.current.isPendingDismiss));
		let unlockViewportZoom = null;
		(0, solid_js.createEffect)((0, solid_js.on)(isActivated, (activated, previousActivated) => {
		...
		const [isEnabled, setIsEnabled] = (0, solid_js.createSignal)(savedToolbarState?.enabled ?? true);
		const [toolbarShakeCount, setToolbarShakeCount] = (0, solid_js.createSignal)(0);
		const [selectionLabelShakeCount, setSelectionLabelShakeCount] = (0, solid_js.createSignal)(0);
		const [currentToolbarState, setCurrentToolbarState] = (0, solid_js.createSignal)(savedToolbarState);
		const [isToolbarSelectHovered, setIsToolbarSelectHovered] = (0, solid_js.createSignal)(false);
		const [commentItems, setCommentItems] = (0, solid_js.createSignal)(loadComments());
		const [commentsDropdownPosition, setCommentsDropdownPosition] = (0, solid_js.createSignal)(null);
		const [toolbarMenuPosition, setToolbarMenuPosition] = (0, solid_js.createSignal)(null);
		const [clearPromptPosition, setClearPromptPosition] = (0, solid_js.createSignal)(null);
		let toolbarElement;
		let dropdownTrackingFrameId = null;
		const commentElementMap = /* @__PURE__ */ new Map();
		const [clockFlashTrigger, setClockFlashTrigger] = (0, solid_js.createSignal)(0);
		const [isCommentsHoverOpen, setIsCommentsHoverOpen] = (0, solid_js.createSignal)(false);
		...
		const reacquireCommentElements = (commentItem) => {
			...
			for (const selector of selectors) {
				...
				try {
					...
					if (require_state.isElementConnected(reacquiredElement)) reacquiredElements.push(reacquiredElement);
				} catch (error) {
					require_freeze_updates.logRecoverableError("Invalid stored selector", error);
				}
			}
		...
		const getConnectedCommentElements = (commentItem) => {
			...
			const connectedMappedElements = mappedElements.filter((mappedElement) => require_state.isElementConnected(mappedElement));
		...
		const commentsDisconnectedItemIds = (0, solid_js.createMemo)(() => {
		...
		(0, solid_js.createEffect)(() => {
			...
			(0, solid_js.onCleanup)(clearHoldTimer);
		});
		(0, solid_js.createEffect)(() => {
			...
			(0, solid_js.onCleanup)(() => clearTimeout(timerId));
		});
		(0, solid_js.createEffect)(() => {
		...
		(0, solid_js.createEffect)((0, solid_js.on)(isHoldingKeys, (currentlyHolding, previouslyHolding = false) => {
		...
		const [debouncedDragPointer, setDebouncedDragPointer] = (0, solid_js.createSignal)(null);
		...
		const [isInspectMode, setIsInspectMode] = (0, solid_js.createSignal)(false);
		...
		const [debouncedElementForComponentName, setDebouncedElementForComponentName] = (0, solid_js.createSignal)(null);
		const [resolvedComponentName, setResolvedComponentName] = (0, solid_js.createSignal)(void 0);
		const [actionCycleItems, setActionCycleItems] = (0, solid_js.createSignal)([]);
		const [actionCycleActiveIndex, setActionCycleActiveIndex] = (0, solid_js.createSignal)(null);
		const [arrowNavigationElements, setArrowNavigationElements] = (0, solid_js.createSignal)([]);
		const [arrowNavigationActiveIndex, setArrowNavigationActiveIndex] = (0, solid_js.createSignal)(0);
		...
		const isRendererActive = (0, solid_js.createMemo)(() => isActivated() && !isCopying());
		...
		const handleCopySuccessWithComments = (options) => {
			...
			const elementSelectors = copiedElements.map((copiedElement, index) => require_extract_element_css.createElementSelector(copiedElement, index === 0));
		...
		const targetElement = (0, solid_js.createMemo)(() => {
		...
		const effectiveElement = (0, solid_js.createMemo)(() => store.frozenElement || (isFrozenPhase() ? null : targetElement()));
		(0, solid_js.createEffect)(() => {
			const element = store.detectedElement;
			if (!element) return;
		...
		(0, solid_js.createEffect)((0, solid_js.on)(effectiveElement, (element) => {
		...
		(0, solid_js.createEffect)(() => {
			...
			(0, solid_js.onCleanup)(require_freeze_updates.freezeAnimations(elements));
		});
		(0, solid_js.createEffect)((0, solid_js.on)(isActivated, (activated) => {
			if (!activated) return;
			if (!pluginRegistry.store.options.freezeReactUpdates) return;
			(0, solid_js.onCleanup)(require_freeze_updates.freezeUpdates());
		}));
		...
		const selectionElement = (0, solid_js.createMemo)(() => getSelectionElement());
		...
		const frozenElementsBounds = (0, solid_js.createMemo)(() => {
			store.viewportVersion;
		...
		const selectionBounds = (0, solid_js.createMemo)(() => {
			store.viewportVersion;
			const frozenElements = store.frozenElements;
		...
		const isDraggingBeyondThreshold = (0, solid_js.createMemo)(() => {
		...
		const dragBounds = (0, solid_js.createMemo)(() => {
			store.viewportVersion;
		...
		const dragPreviewBounds = (0, solid_js.createMemo)(() => {
			store.viewportVersion;
		...
		(0, solid_js.createEffect)((0, solid_js.on)(() => [targetElement(), store.lastGrabbedElement], ([currentElement, lastElement]) => {
		...
		(0, solid_js.createEffect)((0, solid_js.on)(() => targetElement(), (element) => {
		...
		const publicGrabbedBoxes = (0, solid_js.createMemo)(() => store.grabbedBoxes.map((box) => ({
			id: box.id,
			bounds: box.bounds,
			createdAt: box.createdAt
		})));
		const publicLabelInstances = (0, solid_js.createMemo)(() => store.labelInstances.map((instance) => ({
			id: instance.id,
		...
		(0, solid_js.createEffect)((0, solid_js.on)((0, solid_js.createMemo)(() => {
		...
		(0, solid_js.createEffect)((0, solid_js.on)(() => [dragVisible(), dragBounds()], ([visible, bounds]) => {
		...
		const handleDragSelection = (dragSelectionRect, hasModifierKeyHeld) => {
			...
			require_freeze_updates.freezeAllAnimations(selectedElements);
		...
		const handleSingleClick = (clientX, clientY, hasModifierKeyHeld) => {
			...
			const element = validFrozenElement ?? validKeyboardSelectedElement ?? require_freeze_updates.getElementAtPosition(clientX, clientY) ?? (require_state.isElementConnected(store.detectedElement) ? store.detectedElement : null);
			if (!element) return;
		...
		const arrowNavigationItems = (0, solid_js.createMemo)(() => arrowNavigationElements().map((element) => ({
		...
		const arrowNavigationState = (0, solid_js.createMemo)(() => ({
			items: arrowNavigationItems(),
			activeIndex: arrowNavigationActiveIndex(),
			isVisible: arrowNavigationElements().length > 0
		}));
		const [resolvedHookState, setResolvedHookState] = (0, solid_js.createSignal)(void 0);
		(0, solid_js.createEffect)((0, solid_js.on)(() => isInspectMode() ? effectiveElement() : null, (element) => {
			setResolvedHookState(void 0);
			if (!element) return;
			initRenderTimeline();
			resolveHookNames(element).then((namedRows) => {
				if (namedRows && effectiveElement() === element) setResolvedHookState({
					forElement: element,
					rows: namedRows
				});
			}).catch(() => {});
		}));
		const inspectPropertiesState = (0, solid_js.createMemo)(() => {
			if (!isInspectMode()) return void 0;
			const element = effectiveElement();
			if (!element) return void 0;
			...
			if (!(source || reactProps.length > 0 || hooks.length > 0 || timeline && timeline.commits.length > 0)) return void 0;
			return {
				reactProps,
				hooks,
				source,
		...
		(0, solid_js.createEffect)((0, solid_js.on)(selectionElement, () => {
			resetActionCycle();
		}));
		(0, solid_js.createEffect)((0, solid_js.on)(canCycleActions, (isEnabled) => {
		...
		const availableActionCycleItems = (0, solid_js.createMemo)(() => {
		...
		const scheduleActionCycleActivation = () => {
			...
			actionCycleIdleTimeoutId = window.setTimeout(() => {
				...
				if (!context || !require_state.resolveActionEnabled(action, context)) {
					resetActionCycle();
					return;
				}
				resetActionCycle();
		...
		eventListenerManager.addWindowListener("pointermove", (event) => {
			...
			if ((isTouchPointer ? isHoldingKeys() : isActivated()) && !isPromptMode() && !isInspectMode() && isFrozenPhase()) {
				actions.unfreeze();
				clearArrowNavigation();
			}
		...
		eventListenerManager.addWindowListener("contextmenu", (event) => {
			...
			event.preventDefault();
			event.stopPropagation();
			const element = require_freeze_updates.getElementAtPosition(event.clientX, event.clientY);
			if (!element) return;
		...
		const redetectElementUnderPointer = () => {
			...
			if (isEnabled() && !isPromptMode() && !isFrozenPhase() && !isDragging() && store.contextMenuPosition === null && store.frozenElements.length === 0) {
				const candidate = require_freeze_updates.getElementAtPosition(store.pointer.x, store.pointer.y);
		...
		const rendererRoot = mountRoot(typeof "/*! tailwindcss v4.1.17 | MIT License | https://tailwindcss.com */\n@layer properties{@supports (((-webkit-hyphens:none)) and (not (margin-trim:inline))) or ((-moz-orient:inline) and (not (color:rgb(from red r g b)))){*,:before,:after,::backdrop{--tw-scale-x:1;--tw-scale-y:1;--tw-scale-z:1;--tw-rotate-x:initial;--tw-rotate-y:initial;--tw-rotate-z:initial;--tw-skew-x:initial;--tw-skew-y:initial;--tw-border-style:solid;--tw-leading:initial;--tw-font-weight:initial;--tw-ordinal:initial;--tw-slashed-zero:initial;--tw-numeric-figure:initial;--tw-numeric-spacing:initial;--tw-numeric-fraction:initial;--tw-shadow:0 0 #0000;--tw-shadow-color:initial;--tw-shadow-alpha:100%;--tw-inset-shadow:0 0 #0000;--tw-inset-shadow-color:initial;--tw-inset-shadow-alpha:100%;--tw-ring-color:initial;--tw-ring-shadow:0 0 #0000;--tw-inset-ring-color:initial;--tw-inset-ring-shadow:0 0 #0000;--tw-ring-inset:initial;--tw-ring-offset-width:0px;--tw-ring-offset-color:#fff;--tw-ring-offset-shadow:0 0 #0000;--tw-outline-style:solid;--tw-blur:initial;--tw-brightness:initial;--tw-contrast:initial;--tw-grayscale:initial;--tw-hue-rotate:initial;--tw-invert:initial;--tw-opacity:initial;--tw-saturate:initial;--tw-sepia:initial;--tw-drop-shadow:initial;--tw-drop-shadow-color:initial;--tw-drop-shadow-alpha:100%;--tw-drop-shadow-size:initial;--tw-backdrop-blur:initial;--tw-backdrop-brightness:initial;--tw-backdrop-contrast:initial;--tw-backdrop-grayscale:initial;--tw-backdrop-hue-rotate:initial;--tw-backdrop-invert:initial;--tw-backdrop-opacity:initial;--tw-backdrop-saturate:initial;--tw-backdrop-sepia:initial;--tw-duration:initial;--tw-ease:initial;--tw-contain-size:initial;--tw-contain-layout:initial;--tw-contain-paint:initial;--tw-contain-style:initial;--tw-content:\"\"}}}@layer theme{:root,:host{--font-sans:\"Geist\",ui-sans-serif,system-ui,sans-serif;--font-mono:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,\"Liberation Mono\",\"Courier New\",monospace;--color-black:#000;--color-white:#fff;--spacing:4px;--font-weight-medium:500;--font-weight-semibold:600;--radius-sm:4px;--ease-out:cubic-bezier(0,0,.2,1);--default-transition-duration:.15s;--default-transition-timing-function:cubic-bezier(.4,0,.2,1);--default-font-family:var(--font-sans);--default-mono-font-family:var(--font-mono);--transition-fast:.1s;--transition-normal:.15s}}@layer base{*,:after,:before,::backdrop{box-sizing:border-box;border:0 solid;margin:0;padding:0}::file-selector-button{box-sizing:border-box;border:0 solid;margin:0;padding:0}html,:host{-webkit-text-size-adjust:100%;tab-size:4;line-height:1.5;font-family:var(--default-font-family,ui-sans-serif,system-ui,sans-serif,\"Apple Color Emoji\",\"Segoe UI Emoji\",\"Segoe UI Symbol\",\"Noto Color Emoji\");font-feature-settings:var(--default-font-feature-settings,normal);font-variation-settings:var(--default-font-variation-settings,normal);-webkit-tap-highlight-color:transparent}hr{height:0;color:inherit;border-top-width:1px}abbr:where([title]){-webkit-text-decoration:underline dotted;text-decoration:underline dotted}h1,h2,h3,h4,h5,h6{font-size:inherit;font-weight:inherit}a{color:inherit;-webkit-text-decoration:inherit;-webkit-text-decoration:inherit;-webkit-text-decoration:inherit;text-decoration:inherit}b,strong{font-weight:bolder}code,kbd,samp,pre{font-family:var(--default-mono-font-family,ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,\"Liberation Mono\",\"Courier New\",monospace);font-feature-settings:var(--default-mono-font-feature-settings,normal);font-variation-settings:var(--default-mono-font-variation-settings,normal);font-size:1em}small{font-size:80%}sub,sup{vertical-align:baseline;font-size:75%;line-height:0;position:relative}sub{bottom:-.25em}sup{top:-.5em}table{text-indent:0;border-color:inherit;border-collapse:collapse}:-moz-focusring{outline:auto}progress{vertical-align:baseline}summary{display:list-item}ol,ul,menu{list-style:none}img,svg,video,canvas,audio,iframe,embed,object{vertical-align:middle;display:block}img,video{max-width:100%;height:auto}button,input,select,optgroup,textarea{font:inherit;font-feature-settings:inherit;font-variation-settings:inherit;letter-spacing:inherit;color:inherit;opacity:1;background-color:#0000;border-radius:0}::file-selector-button{font:inherit;font-feature-settings:inherit;font-variation-settings:inherit;letter-spacing:inherit;color:inherit;opacity:1;background-color:#0000;border-radius:0}:where(select:is([multiple],[size])) optgroup{font-weight:bolder}:where(select:is([multiple],[size])) optgroup option{padding-inline-start:20px}::file-selector-button{margin-inline-end:4px}::placeholder{opacity:1}@supports (not ((-webkit-appearance:-apple-pay-button))) or (contain-intrinsic-size:1px){::placeholder{color:currentColor}@supports (color:color-mix(in lab, red, red)){::placeholder{color:color-mix(in oklab,currentcolor 50%,transparent)}}}textarea{resize:vertical}::-webkit-search-decoration{-webkit-appearance:none}::-webkit-date-and-time-value{min-height:1lh;text-align:inherit}::-webkit-datetime-edit{display:inline-flex}::-webkit-datetime-edit-fields-wrapper{padding:0}::-webkit-datetime-edit{padding-block:0}::-webkit-datetime-edit-year-field{padding-block:0}::-webkit-datetime-edit-month-field{padding-block:0}::-webkit-datetime-edit-day-field{padding-block:0}::-webkit-datetime-edit-hour-field{padding-block:0}::-webkit-datetime-edit-minute-field{padding-block:0}::-webkit-datetime-edit-second-field{padding-block:0}::-webkit-datetime-edit-millisecond-field{padding-block:0}::-webkit-datetime-edit-meridiem-field{padding-block:0}::-webkit-calendar-picker-indicator{line-height:1}:-moz-ui-invalid{box-shadow:none}button,input:where([type=button],[type=reset],[type=submit]){appearance:button}::file-selector-button{appearance:button}::-webkit-inner-spin-button{height:auto}::-webkit-outer-spin-button{height:auto}[hidden]:where(:not([hidden=until-found])){display:none!important}}@layer components;@layer utilities{.pointer-events-auto{pointer-events:auto}.pointer-events-none{pointer-events:none}.invisible{visibility:hidden}.visible{visibility:visible}.touch-hitbox{position:relative}.touch-hitbox:before{content:\"\";width:100%;min-width:44px;height:100%;min-height:44px;display:block;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%)}.absolute{position:absolute}.fixed{position:fixed}.relative{position:relative}.-top-1{top:calc(var(--spacing)*-1)}.top-0{top:calc(var(--spacing)*0)}.top-0\\.5{top:calc(var(--spacing)*.5)}.top-full{top:100%}.-right-1{right:calc(var(--spacing)*-1)}.right-full{right:100%}.bottom-full{bottom:100%}.left-0{left:calc(var(--spacing)*0)}.left-0\\.5{left:calc(var(--spacing)*.5)}.left-1\\.5{left:calc(var(--spacing)*1.5)}.left-2\\.5{left:calc(var(--spacing)*2.5)}.left-full{left:100%}.z-1{z-index:1}.z-10{z-index:10}.container{width:100%}@media (min-width:640px){.container{max-width:640px}}@media (min-width:768px){.container{max-width:768px}}@media (min-width:1024px){.container{max-width:1024px}}@media (min-width:1280px){.container{max-width:1280px}}@media (min-width:1536px){.container{max-width:1536px}}.m-0{margin:calc(var(--spacing)*0)}.-mx-2{margin-inline:calc(var(--spacing)*-2)}.mx-0\\.5{margin-inline:calc(var(--spacing)*.5)}.-my-1\\.5{margin-block:calc(var(--spacing)*-1.5)}.my-0\\.5{margin-block:calc(var(--spacing)*.5)}.mt-0\\.5{margin-top:calc(var(--spacing)*.5)}.mt-2\\.5{margin-top:calc(var(--spacing)*2.5)}.mr-0\\.5{margin-right:calc(var(--spacing)*.5)}.mr-1\\.5{margin-right:calc(var(--spacing)*1.5)}.mr-2\\.5{margin-right:calc(var(--spacing)*2.5)}.mb-0\\.5{margin-bottom:calc(var(--spacing)*.5)}.mb-1\\.5{margin-bottom:calc(var(--spacing)*1.5)}.mb-2\\.5{margin-bottom:calc(var(--spacing)*2.5)}.-ml-\\[2px\\]{margin-left:-2px}.ml-0\\.5{margin-left:calc(var(--spacing)*.5)}.ml-1{margin-left:calc(var(--spacing)*1)}.ml-2\\.5{margin-left:calc(var(--spacing)*2.5)}.ml-4{margin-left:calc(var(--spacing)*4)}.line-clamp-5{-webkit-line-clamp:5;-webkit-box-orient:vertical;display:-webkit-box;overflow:hidden}.block{display:block}.flex{display:flex}.grid{display:grid}.hidden{display:none}.inline{display:inline}.inline-flex{display:inline-flex}.size-4{width:calc(var(--spacing)*4);height:calc(var(--spacing)*4)}.size-\\[18px\\]{width:18px;height:18px}.h-0{height:calc(var(--spacing)*0)}.h-1\\.5{height:calc(var(--spacing)*1.5)}.h-2{height:calc(var(--spacing)*2)}.h-2\\.5{height:calc(var(--spacing)*2.5)}.h-3{height:calc(var(--spacing)*3)}.h-3\\.5{height:calc(var(--spacing)*3.5)}.h-\\[17px\\]{height:17px}.h-fit{height:fit-content}.max-h-\\[240px\\]{max-height:240px}.min-h-0{min-height:calc(var(--spacing)*0)}.min-h-4{min-height:calc(var(--spacing)*4)}.w-0{width:calc(var(--spacing)*0)}.w-1\\.5{width:calc(var(--spacing)*1.5)}.w-2{width:calc(var(--spacing)*2)}.w-3\\.5{width:calc(var(--spacing)*3.5)}.w-5{width:calc(var(--spacing)*5)}.w-\\[calc\\(100\\%\\+16px\\)\\]{width:calc(100% + 16px)}.w-auto{width:auto}.w-fit{width:fit-content}.w-full{width:100%}.max-w-\\[280px\\]{max-width:280px}.max-w-full{max-width:100%}.min-w-0{min-width:calc(var(--spacing)*0)}.min-w-2\\.5{min-width:calc(var(--spacing)*2.5)}.min-w-\\[100px\\]{min-width:100px}.min-w-\\[150px\\]{min-width:150px}.flex-1{flex:1}.flex-shrink,.shrink{flex-shrink:1}.shrink-0{flex-shrink:0}.flex-grow{flex-grow:1}.scale-75{--tw-scale-x:75%;--tw-scale-y:75%;--tw-scale-z:75%;scale:var(--tw-scale-x)var(--tw-scale-y)}.scale-100{--tw-scale-x:100%;--tw-scale-y:100%;--tw-scale-z:100%;scale:var(--tw-scale-x)var(--tw-scale-y)}.-rotate-90{rotate:-90deg}.rotate-0{rotate:none}.rotate-90{rotate:90deg}.rotate-180{rotate:180deg}.interactive-scale{transition-property:transform;transition-duration:var(--transition-normal);transition-timing-function:cubic-bezier(.34,1.56,.64,1)}@media (hover:hover) and (pointer:fine){.interactive-scale:hover{transform:scale(1.05)}}.interactive-scale:active{transform:scale(.97)}.press-scale{transition-property:transform;transition-duration:var(--transition-fast);transition-timing-function:ease-out}.press-scale:active{transform:scale(.97)}.transform{transform:var(--tw-rotate-x,)var(--tw-rotate-y,)var(--tw-rotate-z,)var(--tw-skew-x,)var(--tw-skew-y,)}.animate-\\[hint-flip-in_var\\(--transition-normal\\)_ease-out\\]{animation:hint-flip-in var(--transition-normal)ease-out}.cursor-grab{cursor:grab}.cursor-grabbing{cursor:grabbing}.cursor-pointer{cursor:pointer}.cursor-text{cursor:text}.resize{resize:both}.resize-none{resize:none}.grid-cols-\\[0fr\\]{grid-template-columns:0fr}.grid-cols-\\[1fr\\]{grid-template-columns:1fr}.grid-rows-\\[0fr\\]{grid-template-rows:0fr}.grid-rows-\\[1fr\\]{grid-template-rows:1fr}.flex-col{flex-direction:column}.flex-wrap{flex-wrap:wrap}.items-center{align-items:center}.items-end{align-items:flex-end}.items-start{align-items:flex-start}.justify-between{justify-content:space-between}.justify-center{justify-content:center}.justify-end{justify-content:flex-end}.gap-0\\.5{gap:calc(var(--spacing)*.5)}.gap-1{gap:calc(var(--spacing)*1)}.gap-1\\.5{gap:calc(var(--spacing)*1.5)}.gap-2{gap:calc(var(--spacing)*2)}.gap-3{gap:calc(var(--spacing)*3)}.gap-\\[5px\\]{gap:5px}.gap-px{gap:1px}.self-stretch{align-self:stretch}.truncate{text-overflow:ellipsis;white-space:nowrap;overflow:hidden}.overflow-hidden{overflow:hidden}.overflow-visible{overflow:visible}.overflow-y-auto{overflow-y:auto}.rounded-\\[3px\\]{border-radius:3px}.rounded-\\[10px\\]{border-radius:10px}.rounded-full{border-radius:3.40282e38px}.rounded-sm{border-radius:var(--radius-sm)}.rounded-t-\\[1px\\]{border-top-left-radius:1px;border-top-right-radius:1px}.rounded-t-\\[10px\\]{border-top-left-radius:10px;border-top-right-radius:10px}.rounded-t-none{border-top-left-radius:0;border-top-right-radius:0}.rounded-l-\\[10px\\]{border-top-left-radius:10px;border-bottom-left-radius:10px}.rounded-l-none{border-top-left-radius:0;border-bottom-left-radius:0}.rounded-r-\\[10px\\]{border-top-right-radius:10px;border-bottom-right-radius:10px}.rounded-r-none{border-top-right-radius:0;border-bottom-right-radius:0}.rounded-b-\\[6px\\]{border-bottom-right-radius:6px;border-bottom-left-radius:6px}.rounded-b-\\[10px\\]{border-bottom-right-radius:10px;border-bottom-left-radius:10px}.rounded-b-none{border-bottom-right-radius:0;border-bottom-left-radius:0}.border{border-style:var(--tw-border-style);border-width:1px}.\\[border-width\\:0\\.5px\\]{border-width:.5px}.\\[border-top-width\\:0\\.5px\\]{border-top-width:.5px}.border-none{--tw-border-style:none;border-style:none}.border-solid{--tw-border-style:solid;border-style:solid}.border-\\[\\#B3B3B3\\]{border-color:#b3b3b3}.border-t-\\[\\#D9D9D9\\]{border-top-color:#d9d9d9}.bg-\\[\\#FEF2F2\\]{background-color:#fef2f2}.bg-black{background-color:var(--color-black)}.bg-black\\/5{background-color:#0000000d}@supports (color:color-mix(in lab, red, red)){.bg-black\\/5{background-color:color-mix(in oklab,var(--color-black)5%,transparent)}}.bg-black\\/25{background-color:#00000040}@supports (color:color-mix(in lab, red, red)){.bg-black\\/25{background-color:color-mix(in oklab,var(--color-black)25%,transparent)}}.bg-transparent{background-color:#0000}.bg-white{background-color:var(--color-white)}.p-0{padding:calc(var(--spacing)*0)}.px-0\\.5{padding-inline:calc(var(--spacing)*.5)}.px-0\\.25{padding-inline:calc(var(--spacing)*.25)}.px-1\\.5{padding-inline:calc(var(--spacing)*1.5)}.px-2{padding-inline:calc(var(--spacing)*2)}.px-\\[3px\\]{padding-inline:3px}.py-0\\.5{padding-block:calc(var(--spacing)*.5)}.py-0\\.25{padding-block:calc(var(--spacing)*.25)}.py-1{padding-block:calc(var(--spacing)*1)}.py-1\\.5{padding-block:calc(var(--spacing)*1.5)}.py-2{padding-block:calc(var(--spacing)*2)}.py-px{padding-block:1px}.pt-0\\.5{padding-top:calc(var(--spacing)*.5)}.pt-1\\.5{padding-top:calc(var(--spacing)*1.5)}.pb-1{padding-bottom:calc(var(--spacing)*1)}.text-left{text-align:left}.text-right{text-align:right}.font-sans{font-family:var(--font-sans)}.text-\\[8px\\]{font-size:8px}.text-\\[10px\\]{font-size:10px}.text-\\[11px\\]{font-size:11px}.text-\\[12px\\]{font-size:12px}.text-\\[13px\\]{font-size:13px}.leading-3{--tw-leading:calc(var(--spacing)*3);line-height:calc(var(--spacing)*3)}.leading-3\\.5{--tw-leading:calc(var(--spacing)*3.5);line-height:calc(var(--spacing)*3.5)}.leading-4{--tw-leading:calc(var(--spacing)*4);line-height:calc(var(--spacing)*4)}.leading-none{--tw-leading:1;line-height:1}.font-medium{--tw-font-weight:var(--font-weight-medium);font-weight:var(--font-weight-medium)}.font-semibold{--tw-font-weight:var(--font-weight-semibold);font-weight:var(--font-weight-semibold)}.wrap-break-word{overflow-wrap:break-word}.text-ellipsis{text-overflow:ellipsis}.whitespace-nowrap{white-space:nowrap}.text-\\[\\#71717a\\]{color:#71717a}.text-\\[\\#B3B3B3\\]{color:#b3b3b3}.text-\\[\\#B91C1C\\]{color:#b91c1c}.text-\\[\\#B91C1C\\]\\/50{color:oklab(50.542% .168942 .0880134/.5)}.text-black{color:var(--color-black)}.text-black\\/25{color:#00000040}@supports (color:color-mix(in lab, red, red)){.text-black\\/25{color:color-mix(in oklab,var(--color-black)25%,transparent)}}.text-black\\/30{color:#0000004d}@supports (color:color-mix(in lab, red, red)){.text-black\\/30{color:color-mix(in oklab,var(--color-black)30%,transparent)}}.text-black\\/40{color:#0006}@supports (color:color-mix(in lab, red, red)){.text-black\\/40{color:color-mix(in oklab,var(--color-black)40%,transparent)}}.text-black\\/50{color:#00000080}@supports (color:color-mix(in lab, red, red)){.text-black\\/50{color:color-mix(in oklab,var(--color-black)50%,transparent)}}.text-black\\/60{color:#0009}@supports (color:color-mix(in lab, red, red)){.text-black\\/60{color:color-mix(in oklab,var(--color-black)60%,transparent)}}.text-black\\/70{color:#000000b3}@supports (color:color-mix(in lab, red, red)){.text-black\\/70{color:color-mix(in oklab,var(--color-black)70%,transparent)}}.text-black\\/85{color:#000000d9}@supports (color:color-mix(in lab, red, red)){.text-black\\/85{color:color-mix(in oklab,var(--color-black)85%,transparent)}}.text-white{color:var(--color-white)}.tabular-nums{--tw-numeric-spacing:tabular-nums;font-variant-numeric:var(--tw-ordinal,)var(--tw-slashed-zero,)var(--tw-numeric-figure,)var(--tw-numeric-spacing,)var(--tw-numeric-fraction,)}.antialiased{-webkit-font-smoothing:antialiased;-moz-osx-font-smoothing:grayscale}.opacity-0{opacity:0}.opacity-40{opacity:.4}.opacity-100{opacity:1}.shadow{--tw-shadow:0 1px 3px 0 var(--tw-shadow-color,#0000001a),0 1px 2px -1px var(--tw-shadow-color,#0000001a);box-shadow:var(--tw-inset-shadow),var(--tw-inset-ring-shadow),var(--tw-ring-offset-shadow),var(--tw-ring-shadow),var(--tw-shadow)}.outline{outline-style:var(--tw-outline-style);outline-width:1px}.blur{--tw-blur:blur(8px);filter:var(--tw-blur,)var(--tw-brightness,)var(--tw-contrast,)var(--tw-grayscale,)var(--tw-hue-rotate,)var(--tw-invert,)var(--tw-saturate,)var(--tw-sepia,)var(--tw-drop-shadow,)}.filter{filter:var(--tw-blur,)var(--tw-brightness,)var(--tw-contrast,)var(--tw-grayscale,)var(--tw-hue-rotate,)var(--tw-invert,)var(--tw-saturate,)var(--tw-sepia,)var(--tw-drop-shadow,)}.filter-\\[drop-shadow\\(0px_1px_2px_\\#51515140\\)\\]{filter:drop-shadow(0 1px 2px #51515140)}.backdrop-filter{-webkit-backdrop-filter:var(--tw-backdrop-blur,)var(--tw-backdrop-brightness,)var(--tw-backdrop-contrast,)var(--tw-backdrop-grayscale,)var(--tw-backdrop-hue-rotate,)var(--tw-backdrop-invert,)var(--tw-backdrop-opacity,)var(--tw-backdrop-saturate,)var(--tw-backdrop-sepia,);backdrop-filter:var(--tw-backdrop-blur,)var(--tw-backdrop-brightness,)var(--tw-backdrop-contrast,)var(--tw-backdrop-grayscale,)var(--tw-backdrop-hue-rotate,)var(--tw-backdrop-invert,)var(--tw-backdrop-opacity,)var(--tw-backdrop-saturate,)var(--tw-backdrop-sepia,)}.transition{transition-property:color,background-color,border-color,outline-color,text-decoration-color,fill,stroke,--tw-gradient-from,--tw-gradient-via,--tw-gradient-to,opacity,box-shadow,transform,translate,scale,rotate,filter,-webkit-backdrop-filter,backdrop-filter,display,content-visibility,overlay,pointer-events;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-\\[grid-template-columns\\,opacity\\]{transition-property:grid-template-columns,opacity;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-\\[grid-template-columns\\]{transition-property:grid-template-columns;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-\\[grid-template-rows\\,opacity\\]{transition-property:grid-template-rows,opacity;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-\\[grid-template-rows\\]{transition-property:grid-template-rows;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-\\[opacity\\,transform\\]{transition-property:opacity,transform;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-\\[top\\,left\\,width\\,height\\,opacity\\]{transition-property:top,left,width,height,opacity;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-\\[transform\\,opacity\\]{transition-property:transform,opacity;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-all{transition-property:all;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-colors{transition-property:color,background-color,border-color,outline-color,text-decoration-color,fill,stroke,--tw-gradient-from,--tw-gradient-via,--tw-gradient-to;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-opacity{transition-property:opacity;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.transition-transform{transition-property:transform,translate,scale,rotate;transition-timing-function:var(--tw-ease,var(--default-transition-timing-function));transition-duration:var(--tw-duration,var(--default-transition-duration))}.duration-75{--tw-duration:75ms;transition-duration:75ms}.duration-100{--tw-duration:.1s;transition-duration:.1s}.duration-150{--tw-duration:.15s;transition-duration:.15s}.duration-300{--tw-duration:.3s;transition-duration:.3s}.ease-out{--tw-ease:var(--ease-out);transition-timing-function:var(--ease-out)}.will-change-\\[opacity\\,transform\\]{will-change:opacity,transform}.contain-layout{--tw-contain-layout:layout;contain:var(--tw-contain-size,)var(--tw-contain-layout,)var(--tw-contain-paint,)var(--tw-contain-style,)}.outline-none{--tw-outline-style:none;outline-style:none}.select-none{-webkit-user-select:none;user-select:none}.select-text{-webkit-user-select:text;user-select:text}.\\[animation-fill-mode\\:backwards\\]{animation-fill-mode:backwards}.\\[corner-shape\\:superellipse\\(1\\.25\\)\\]{corner-shape:superellipse(1.25)}.\\[font-synthesis\\:none\\]{font-synthesis:none}.\\[scrollbar-color\\:transparent_transparent\\]{scrollbar-color:transparent transparent}.\\[scrollbar-width\\:thin\\]{scrollbar-width:thin}.before\\:\\!min-h-full:before{content:var(--tw-content);min-height:100%!important}.before\\:\\!min-w-full:before{content:var(--tw-content);min-width:100%!important}@media (hover:hover){.hover\\:bg-\\[\\#F5F5F5\\]:hover{background-color:#f5f5f5}.hover\\:bg-\\[\\#FEE2E2\\]:hover{background-color:#fee2e2}.hover\\:bg-black\\/10:hover{background-color:#0000001a}@supports (color:color-mix(in lab, red, red)){.hover\\:bg-black\\/10:hover{background-color:color-mix(in oklab,var(--color-black)10%,transparent)}}.hover\\:text-black:hover{color:var(--color-black)}.hover\\:opacity-100:hover{opacity:1}.hover\\:\\[scrollbar-color\\:rgba\\(0\\,0\\,0\\,0\\.15\\)_transparent\\]:hover{scrollbar-color:#00000026 transparent}}.disabled\\:cursor-default:disabled{cursor:default}.disabled\\:opacity-40:disabled{opacity:.4}}:host{all:initial;direction:ltr}@keyframes shake{0%,to{transform:translate(0)}15%{transform:translate(-3px)}30%{transform:translate(3px)}45%{transform:translate(-3px)}60%{transform:translate(3px)}75%{transform:translate(-2px)}90%{transform:translate(2px)}}@keyframes success-pop{0%{opacity:0;transform:scale(.9)}60%{opacity:1;transform:scale(1.1)}80%{transform:scale(.95)}to{opacity:1;transform:scale(1)}}@keyframes hint-flip-in{0%{opacity:0;transform:translateY(4px)}to{opacity:1;transform:translateY(0)}}@keyframes tooltip-fade-in{0%{opacity:0;transform:scale(.97)}to{opacity:1;transform:scale(1)}}@keyframes icon-loader-spin{0%{opacity:1}50%{opacity:.5}to{opacity:.2}}.icon-loader-bar{animation:.5s linear infinite icon-loader-spin}@keyframes shimmer{0%{background-position:200% 0}to{background-position:-200% 0}}.shimmer-text{color:#0000;background:linear-gradient(90deg,#71717a 0%,#a1a1aa 25%,#71717a 50%,#a1a1aa 75%,#71717a 100%) 0 0/200% 100%;-webkit-background-clip:text;background-clip:text;animation:2.5s linear infinite shimmer}@keyframes clock-flash{0%{transform:scale(1)}25%{transform:scale(1.2)}50%{transform:scale(.92)}75%{transform:scale(1.05)}to{transform:scale(1)}}.animate-clock-flash{will-change:transform;animation:.4s ease-out clock-flash}.animate-shake{will-change:transform;animation:.3s ease-out shake}.animate-success-pop{will-change:transform,opacity;animation:.25s ease-out success-pop}.animate-tooltip-fade-in{animation:tooltip-fade-in var(--transition-fast)ease-out;will-change:transform,opacity}@property --tw-scale-x{syntax:\"*\";inherits:false;initial-value:1}@property --tw-scale-y{syntax:\"*\";inherits:false;initial-value:1}@property --tw-scale-z{syntax:\"*\";inherits:false;initial-value:1}@property --tw-rotate-x{syntax:\"*\";inherits:false}@property --tw-rotate-y{syntax:\"*\";inherits:false}@property --tw-rotate-z{syntax:\"*\";inherits:false}@property --tw-skew-x{syntax:\"*\";inherits:false}@property --tw-skew-y{syntax:\"*\";inherits:false}@property --tw-border-style{syntax:\"*\";inherits:false;initial-value:solid}@property --tw-leading{syntax:\"*\";inherits:false}@property --tw-font-weight{syntax:\"*\";inherits:false}@property --tw-ordinal{syntax:\"*\";inherits:false}@property --tw-slashed-zero{syntax:\"*\";inherits:false}@property --tw-numeric-figure{syntax:\"*\";inherits:false}@property --tw-numeric-spacing{syntax:\"*\";inherits:false}@property --tw-numeric-fraction{syntax:\"*\";inherits:false}@property --tw-shadow{syntax:\"*\";inherits:false;initial-value:0 0 #0000}@property --tw-shadow-color{syntax:\"*\";inherits:false}@property --tw-shadow-alpha{syntax:\"<percentage>\";inherits:false;initial-value:100%}@property --tw-inset-shadow{syntax:\"*\";inherits:false;initial-value:0 0 #0000}@property --tw-inset-shadow-color{syntax:\"*\";inherits:false}@property --tw-inset-shadow-alpha{syntax:\"<percentage>\";inherits:false;initial-value:100%}@property --tw-ring-color{syntax:\"*\";inherits:false}@property --tw-ring-shadow{syntax:\"*\";inherits:false;initial-value:0 0 #0000}@property --tw-inset-ring-color{syntax:\"*\";inherits:false}@property --tw-inset-ring-shadow{syntax:\"*\";inherits:false;initial-value:0 0 #0000}@property --tw-ring-inset{syntax:\"*\";inherits:false}@property --tw-ring-offset-width{syntax:\"<length>\";inherits:false;initial-value:0}@property --tw-ring-offset-color{syntax:\"*\";inherits:false;initial-value:#fff}@property --tw-ring-offset-shadow{syntax:\"*\";inherits:false;initial-value:0 0 #0000}@property --tw-outline-style{syntax:\"*\";inherits:false;initial-value:solid}@property --tw-blur{syntax:\"*\";inherits:false}@property --tw-brightness{syntax:\"*\";inherits:false}@property --tw-contrast{syntax:\"*\";inherits:false}@property --tw-grayscale{syntax:\"*\";inherits:false}@property --tw-hue-rotate{syntax:\"*\";inherits:false}@property --tw-invert{syntax:\"*\";inherits:false}@property --tw-opacity{syntax:\"*\";inherits:false}@property --tw-saturate{syntax:\"*\";inherits:false}@property --tw-sepia{syntax:\"*\";inherits:false}@property --tw-drop-shadow{syntax:\"*\";inherits:false}@property --tw-drop-shadow-color{syntax:\"*\";inherits:false}@property --tw-drop-shadow-alpha{syntax:\"<percentage>\";inherits:false;initial-value:100%}@property --tw-drop-shadow-size{syntax:\"*\";inherits:false}@property --tw-backdrop-blur{syntax:\"*\";inherits:false}@property --tw-backdrop-brightness{syntax:\"*\";inherits:false}@property --tw-backdrop-contrast{syntax:\"*\";inherits:false}@property --tw-backdrop-grayscale{syntax:\"*\";inherits:false}@property --tw-backdrop-hue-rotate{syntax:\"*\";inherits:false}@property --tw-backdrop-invert{syntax:\"*\";inherits:false}@property --tw-backdrop-opacity{syntax:\"*\";inherits:false}@property --tw-backdrop-saturate{syntax:\"*\";inherits:false}@property --tw-backdrop-sepia{syntax:\"*\";inherits:false}@property --tw-duration{syntax:\"*\";inherits:false}@property --tw-ease{syntax:\"*\";inherits:false}@property --tw-contain-size{syntax:\"*\";inherits:false}@property --tw-contain-layout{syntax:\"*\";inherits:false}@property --tw-contain-paint{syntax:\"*\";inherits:false}@property --tw-contain-style{syntax:\"*\";inherits:false}@property --tw-content{syntax:\"*\";inherits:false;initial-value:\"\"}" === "string" ? styles_default : "");
		const isThemeEnabled = (0, solid_js.createMemo)(() => pluginRegistry.store.theme.enabled);
		const isSelectionBoxThemeEnabled = (0, solid_js.createMemo)(() => pluginRegistry.store.theme.selectionBox.enabled);
		const isElementLabelThemeEnabled = (0, solid_js.createMemo)(() => pluginRegistry.store.theme.elementLabel.enabled);
		const isDragBoxThemeEnabled = (0, solid_js.createMemo)(() => pluginRegistry.store.theme.dragBox.enabled);
		const isSelectionSuppressed = (0, solid_js.createMemo)(() => didJustCopy() || isToolbarSelectHovered() && !isFrozenPhase());
		const hasDragPreviewBounds = (0, solid_js.createMemo)(() => dragPreviewBounds().length > 0);
		const selectionVisible = (0, solid_js.createMemo)(() => {
		...
		const selectionTagName = (0, solid_js.createMemo)(() => {
			const element = selectionElement();
			if (!element) return void 0;
			return require_freeze_updates.getTagName(element) || void 0;
		});
		(0, solid_js.createEffect)((0, solid_js.on)(() => debouncedElementForComponentName(), (element) => {
		...
		const selectionLabelVisible = (0, solid_js.createMemo)(() => {
		...
		const computedLabelInstances = (0, solid_js.createMemo)(() => {
		...
		const computedGrabbedBoxes = (0, solid_js.createMemo)(() => {
			if (!isThemeEnabled()) return [];
			if (!pluginRegistry.store.theme.grabbedBoxes.enabled) return [];
			store.viewportVersion;
		...
		const dragVisible = (0, solid_js.createMemo)(() => isThemeEnabled() && isDragBoxThemeEnabled() && isRendererActive() && isDraggingBeyondThreshold());
		const labelVariant = (0, solid_js.createMemo)(() => isCopying() ? "processing" : "hover");
		const labelVisible = (0, solid_js.createMemo)(() => {
			if (!isThemeEnabled()) return false;
		...
		const contextMenuBounds = (0, solid_js.createMemo)(() => {
			store.viewportVersion;
			const element = store.contextMenuElement;
			if (!element) return null;
			return createElementBounds(element);
		});
		const contextMenuPosition = (0, solid_js.createMemo)(() => {
			store.viewportVersion;
			return store.contextMenuPosition;
		});
		const contextMenuTagName = (0, solid_js.createMemo)(() => {
			const element = store.contextMenuElement;
			if (!element) return void 0;
		...
		const [contextMenuComponentName] = (0, solid_js.createResource)(() => ({
		...
		const [contextMenuFilePath] = (0, solid_js.createResource)(() => store.contextMenuElement, async (element) => {
			if (!element) return null;
		...
		const contextMenuActionContext = (0, solid_js.createMemo)(() => {
			const element = store.contextMenuElement;
			if (!element) return void 0;
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-BQC-FNaQ.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
			disposeRenderer = (0, solid_js_web.render)(() => {
				return (0, solid_js_web.createComponent)(ReactGrabRenderer, {
					...
					get mouseX() {
						return (0, solid_js_web.memo)(() => store.frozenElements.length > 1)() ? void 0 : cursorPosition().x;
					},
					get isFrozen() {
					...
					get isCommentsPinned() {
						return (0, solid_js_web.memo)(() => commentsDropdownPosition() !== null)() && !isCommentsHoverOpen();
					},
