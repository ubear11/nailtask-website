...
const require_extract_element_css = require('./extract-element-css-CD24H1Dj.cjs');
...
const initRenderTimeline = () => {
	...
	require_freeze_updates.Me({ onCommitFiberRoot: (_rendererID, root) => {
		const componentDurations = /* @__PURE__ */ new Map();
...
const getTimelineForElement = (element) => {
	...
	const commits = commitBuffer.filter((entry) => entry.componentDurations.has(componentType)).map((entry) => ({
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		const [resolvedHookRows, setResolvedHookRows] = (0, solid_js.createSignal)(void 0);
		(0, solid_js.createEffect)((0, solid_js.on)(() => isInspectMode() ? effectiveElement() : null, (element) => {
			setResolvedHookRows(void 0);
			if (!element) return;
			resolveHookNames(element).then((rows) => {
				if (rows && effectiveElement() === element) setResolvedHookRows({
					element,
					rows
				});
			});
		...
		const inspectPropertiesState = (0, solid_js.createMemo)(() => {
			...
			const hooks = resolved && resolved.element === element ? resolved.rows : getHooksState(element);
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-piOh5kJL.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
