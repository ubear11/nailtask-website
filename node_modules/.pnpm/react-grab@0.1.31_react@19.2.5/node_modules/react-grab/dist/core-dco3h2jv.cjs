...
const require_extract_element_css = require('./extract-element-css-CstrLg04.cjs');
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-B622bS5P.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
