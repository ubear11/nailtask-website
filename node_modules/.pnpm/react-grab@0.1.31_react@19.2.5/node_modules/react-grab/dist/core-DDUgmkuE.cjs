...
const require_extract_element_css = require('./extract-element-css-7mFk3gvb.cjs');
...
const init = (rawOptions) => {
	...
	return (0, solid_js.createRoot)((dispose) => {
		...
		if (pluginRegistry.store.theme.enabled) Promise.resolve().then(() => require("./renderer-DuM68uGp.cjs")).then(({ ReactGrabRenderer }) => {
			if (disposed) return;
